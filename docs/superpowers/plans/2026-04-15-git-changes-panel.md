...
### Task 6: 前端 — 重构 GitHistory 为 tab 容器
...
- 组件重命名为 `GitHistoryContent`
- 新增 props 接收 `repoPath`（当前选中的仓库路径）和 `projectPath`
- 移除组件内的 "无项目" 空状态判断（由外层容器处理）
- 移除面板标题栏（"Git History" 标题和刷新按钮），由外层容器处理
- **保留所有业务逻辑**（repos 发现、commits 加载、branches、pull/push、PTY 监听、CommitDiffModal 等），不改行为
...
interface GitHistoryContentProps {
  projectPath: string;
  onRefreshNeeded?: () => void; // commit 成功后通知刷新
}
```

- [ ] **Step 2: 改造 `GitHistory.tsx` 为 tab 容器**
  ...
  return (
    <div className="h-full bg-[var(--bg-surface)] flex flex-col border-t border-[var(--border-subtle)]">
      ...
      <div className="flex-1 overflow-hidden">
        {activeTab === 'history' ? (
          <GitHistoryContent projectPath={project.path} />
        ) : (
...
注意：此时 `GitChanges` 组件尚未创建，可以先放一个占位符 export 或者此 Task 和 Task 7 一起完成。

- [ ] **Step 3: 验证前端编译**

Run: `cd D:/Git/mini-term && npx tsc --noEmit 2>&1 | tail -10`
注意：如果 `GitChanges` 尚未创建，可先创建一个空壳组件：

```typescript
// src/components/GitChanges.tsx (临时占位)
export function GitChanges({ projectPath }: { projectPath: string }) {
  return (
...
git commit -m "refactor: GitHistory 拆分为 tab 容器 + GitHistoryContent，新增 GitChanges 占位组件"
```
...
import type { ChangeFileStatus, GitRepoInfo, PtyOutputPayload } from '../types';
...
export function GitChanges({ projectPath }: GitChangesProps) {
  ...
  // 加载仓库列表
  useEffect(() => {
    invoke<GitRepoInfo[]>('discover_git_repos', { projectPath })
      .then((r) => {
        setRepos(r);
        if (r.length > 0 && !selectedRepo) {
          setSelectedRepo(r[0].path);
        }
      })
      .catch(() => setRepos([]));
  }, [projectPath]);
  ...
  const loadChanges = useCallback(() => {
    ...
    setLoading(true);
    invoke<ChangeFileStatus[]>('get_changes_status', { repoPath: selectedRepo })
      .then(setChanges)
      .catch(() => setChanges([]))
...
```typescript
  const handleStage = useCallback(async (files: string[]) => {
    try {
      await invoke('git_stage', { repoPath: selectedRepo, files });
      loadChanges();
    } catch (e) {
      console.error('stage failed:', e);
    }
  }, [selectedRepo, loadChanges]);

  const handleUnstage = useCallback(async (files: string[]) => {
    try {
      await invoke('git_unstage', { repoPath: selectedRepo, files });
      loadChanges();
    } catch (e) {
  ...
  const handleStageAll = useCallback(async () => {
    try {
      await invoke('git_stage_all', { repoPath: selectedRepo });
      loadChanges();
    } catch (e) {
  ...
  const handleUnstageAll = useCallback(async () => {
    try {
      await invoke('git_unstage_all', { repoPath: selectedRepo });
      loadChanges();
    } catch (e) {
  ...
  const handleCommit = useCallback(async () => {
    ...
    setCommitting(true);
    try {
      await invoke('git_commit', { repoPath: selectedRepo, message: commitMsg.trim() });
      setCommitMsg('');
      loadChanges();
    } catch (e) {
  ...
  }, [selectedRepo, commitMsg, staged.length, loadChanges]);

  const handleDiscard = useCallback(async (files: string[]) => {
    if (!confirm(`确定要丢弃 ${files.length} 个文件的修改？此操作不可撤销。`)) return;
    try {
      await invoke('git_discard_file', { repoPath: selectedRepo, files });
      loadChanges();
    } catch (e) {
...
- [ ] **Step 3: JSX 渲染 — 文件列表和提交区域**

```typescript
  // 文件行渲染
  const renderFileRow = (file: ChangeFileStatus, area: 'staged' | 'unstaged' | 'untracked') => {
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="truncate" title={file.path}>
            {viewMode === 'list' ? file.path : fileName}
          </span>
        </div>
        <button
  ...
  const renderGroup = (
    ...
    return (
      <div className="mb-2">
        ...
        {files.map((f) => renderFileRow(f, area))}
      </div>
    );
  };
